  ...
  return (
    <div className="space-y-2.5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        ...
        {/* Mode Switcher Pill */}
        <div className="inline-flex rounded-xl bg-slate-100 p-0.5 border border-slate-200 text-[11px] font-bold self-start sm:self-auto">
          <button
            type="button"
      ...
      {inputMode === 'url' ? (
        <div className="space-y-1.5">
          <div className="relative">
            <input
              type="text"
              className="w-full pl-9 pr-24 py-2.5 rounded-xl border border-slate-300 text-xs font-mono font-medium focus:ring-2 focus:ring-rose-500 focus:border-rose-500 transition"
          ...
          <div className="flex items-center gap-2 text-[11px]">
            {videoType === 'youtube' && (
              <span className="inline-flex items-center gap-1 text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-full font-bold border border-emerald-200">
                <CheckCircle2 className="w-3 h-3 text-emerald-600" /> YouTube Video (ID: {videoInfo.id})
              </span>
            )}
            {videoType === 'vimeo' && (
              <span className="inline-flex items-center gap-1 text-sky-700 bg-sky-50 px-2.5 py-0.5 rounded-full font-bold border border-sky-200">
                <CheckCircle2 className="w-3 h-3 text-sky-600" /> Vimeo Video (ID: {videoInfo.id})
              </span>
            )}
            {videoType === 'direct' && value && (
              <span className="inline-flex items-center gap-1 text-indigo-700 bg-indigo-50 px-2.5 py-0.5 rounded-full font-bold border border-indigo-200">
                <FileVideo className="w-3 h-3 text-indigo-600" /> Direct Video File (MP4 / WebM / CDN)
              </span>
            )}
          </div>
        </div>
