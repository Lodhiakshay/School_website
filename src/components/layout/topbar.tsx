...
export const Topbar: React.FC<TopbarProps> = ({ onToggleMobileMenu }) => {
  ...
  return (
    <header className="h-16 sm:h-20 bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-3 sm:px-8 flex items-center justify-between flex-shrink-0 z-20 shadow-sm sticky top-0">
      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
        {onToggleMobileMenu && (
          <button
            onClick={onToggleMobileMenu}
            aria-label="Toggle Menu"
            className="lg:hidden p-2 rounded-xl text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition flex-shrink-0"
          >
        ...
        {/* Session Indicator */}
        <div className="flex items-center gap-1.5 bg-blue-50/90 text-blue-900 px-2.5 sm:px-3.5 py-1 rounded-full text-[11px] sm:text-xs font-bold border border-blue-200/60 shadow-sm flex-shrink-0">
      ...
      <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-bold bg-slate-900 hover:bg-slate-800 text-white px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-xl transition shadow-sm whitespace-nowrap"
          >
            <Sparkles className="w-3.5 h-3.5 text-amber-400 flex-shrink-0" />
            <span className="hidden sm:inline">1-Click Roles</span>
        ...
        <Badge variant="purple" size="sm" className="hidden sm:inline-flex font-extrabold">
          {user?.role || 'Staff'}
        </Badge>
