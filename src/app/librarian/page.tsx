...
export default function LibrarianDashboardPage() {
  ...
  const handleCreateBook = (e: React.FormEvent) => {
    ...
    toast.success(`"${created.title}" added to central library catalog.`, 'Book Cataloged');
  };
  ...
  const handleIssueBook = (e: React.FormEvent) => {
    ...
    toast.success(
      `Issued "${issueForm.bookTitle}" to ${issueForm.studentName}. Due return: ${issueForm.dueDate}.`,
  ...
  return (
    <PortalLayout allowedRoles={['Librarian', 'SuperAdmin', 'Admin', 'Principal']}>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          <div>
            <h1 className="text-lg sm:text-xl font-black text-slate-900 flex items-center gap-2 font-serif">
              <Library className="w-5 h-5 text-blue-600" /> Central Knowledge Library Hub
            </h1>
            <p className="text-xs text-slate-500 mt-0.5">
              5,000+ reference volumes, science journals, Vedic literature, and barcode circulation desk.
            </p>
          </div>
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={() => {
                window.print();
                toast.success('Generated printable Library Catalog sheet.', 'Catalog Ready');
              }}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-200 shadow-sm">
            <span className="text-[10px] font-bold text-emerald-700 uppercase">Available on Shelves</span>
            <div className="text-xl font-black text-emerald-700 mt-1">4,892 Copies</div>
          </div>
          <div className="bg-blue-50 p-4 rounded-2xl border border-blue-200 shadow-sm">
            <span className="text-[10px] font-bold text-blue-700 uppercase">Issued to Students</span>
            <div className="text-xl font-black text-blue-700 mt-1">348 Issued</div>
          </div>
          <div className="bg-amber-50 p-4 rounded-2xl border border-amber-200 shadow-sm">
            ...
            <div className="text-xl font-black text-amber-700 mt-1">12 Pending</div>
          </div>
        </div>
              ...
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 font-bold uppercase text-[10px]">
                  <tr>
                    ...
                    <th className="px-4 py-3.5 text-right">Quick Issue</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
                  {filteredBooks.map((b) => (
                    <tr key={b._id} className="hover:bg-slate-50 transition">
                      <td className="px-4 py-3.5 font-mono font-bold text-blue-600">{b.isbn}</td>
                      <td className="px-4 py-3.5">
                        <div className="font-bold text-slate-900">{b.title}</div>
                      </td>
                      <td className="px-4 py-3.5 text-slate-600">{b.author}</td>
                      <td className="px-4 py-3.5">
                      ...
                      <td className="px-4 py-3.5">
                        <span className="font-black text-emerald-600">{b.availableCopies}</span>
